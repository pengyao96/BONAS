...
def mutate_adj(old_adj):
    ...
    to_disconnect = random.choice(possible_disconnects) if possible_disconnects else \
                                                                    np.nonzero(new_adj[node_to_disconnect])[0][0]
...
def mutate_arch(old_arch):
    ...
    if change == 0:
        ...
        new_arch = {'adjacency_matrix': mutate_adj(old_arch['adjacency_matrix']),
                    "operations": mutate_ops(old_arch['operations'])}
